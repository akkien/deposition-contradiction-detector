...
describe.skipIf(!process.env.GEMINI_API_KEY)('extractClaims — integration (real Gemini)', () => {
  it('extracts claims from prototype T1 transcript', async () => {
    const claims = await extractClaims(TRANSCRIPT_1, 'T1')
    expect(claims.length).toBeGreaterThan(0)
    expect(claims[0].id).toMatch(/^T1_C\d+$/)
    for (const c of claims) {
      expect(c.topic).toBeTruthy()
      expect(c.text).toBeTruthy()
      expect(c.raw_quote).toBeTruthy()
    }
    console.log(`T1: ${claims.length} claims extracted`)
    console.log(JSON.stringify(claims, null, 2))
  }, 30000)

  it('extracts claims from prototype T2 transcript', async () => {
    const claims = await extractClaims(TRANSCRIPT_2, 'T2')
    expect(claims.length).toBeGreaterThan(0)
    expect(claims[0].id).toMatch(/^T2_C\d+$/)
    for (const c of claims) {
      expect(c.topic).toBeTruthy()
      expect(c.text).toBeTruthy()
      expect(c.raw_quote).toBeTruthy()
    }
    console.log(`T2: ${claims.length} claims extracted`)
    console.log(JSON.stringify(claims, null, 2))
  }, 30000)
})
